...
func main() {
	if len(os.Args) != 3 {
		fmt.Println("Usage: compare_stores <data_dir_1> <data_dir_2>")
		os.Exit(1)
	}

	// Open both databases
	db1, err := dbm.NewDB("application", dbm.GoLevelDBBackend, os.Args[1])
	if err != nil {
		panic(err)
	}
	defer db1.Close()

	db2, err := dbm.NewDB("application", dbm.GoLevelDBBackend, os.Args[2])
	if err != nil {
		panic(err)
	}
	defer db2.Close()
