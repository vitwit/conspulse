...
import { BitArrayCandles } from "./components/BitArrayCandles";
import { ErrorAlert } from "./components/ErrorAlert";
import { formatLatency, timeAgo } from "./utils";
import { useWebSocket } from './../context/WebsocketContext';
import { NetworkMessage, Stats } from "../types/ws";
import { SupportUS } from "../components/SupportUs";
import LastBlockConsensusTab from "./components/LastBlockConsensus";
import PeersTableTab from "./components/PeersTableTab";
import { SimpleErrorAlert } from "../components/SimpleErrorAlert";
...
export default function NetstatsPage() {
  ...
  const metrics = [
    ...
    {
      title: "Voting Power",
      value: proposerObj?.voting_power?.toLocaleString() ?? "—",
      icon: Shield,
      accent: "amber",
    },
  ];
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="flex-1 bg-[#0e1014]">
        <section className=" ml-4 mr-4 mt-2 mx-auto rounded-xl shadow-lg pb-8">
          ...
          <div>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-4 p-4 bg-[#0e1014] text-white font-sans">
              {metrics.map(({ title, value, icon: Icon, accent }) => (
                <div
                  key={title}
        ...
        ${activeTab === tab.key
                      ? "bg-[#0e1014] text-cyan-400 border-b-2 border-cyan-400"
                      : "bg-[#2a2f3a] text-gray-400 hover:text-white hover:bg-[#2f3542]"
                    }`}
                >
                  {tab.label}
                </button>
